...
from openai import OpenAI, AzureAuth, AsyncOpenAI, AsyncAzureAuth, APIResponseValidationError
...
class TestOpenAI:
    ...
    def test_refresh_auth_headers_token(self) -> None:
        client = OpenAI(base_url=base_url, auth=AzureAuth(token_provider=lambda: "test_bearer_token"))
    ...
    def test_bearer_token_refresh(self, respx_mock: MockRouter) -> None:
        ...
        client = OpenAI(base_url=base_url, auth=AzureAuth(token_provider=token_provider))
    ...
    def test_auth_mutually_exclusive(self) -> None:
        with pytest.raises(ValueError) as exc_info:
            OpenAI(base_url=base_url, api_key=api_key, auth=AzureAuth(token_provider=lambda: "test_bearer_token"))
        assert str(exc_info.value) == "The `api_key` and `auth` arguments are mutually exclusive"

    def test_copy_auth(self) -> None:
        client = OpenAI(base_url=base_url, auth=AzureAuth(token_provider=lambda: "test_bearer_token_1")).copy(
            auth=AzureAuth(token_provider=lambda: "test_bearer_token_2")
        )
        client.refresh_auth_headers()
    ...
    def test_copy_auth_mutually_exclusive(self) -> None:
        with pytest.raises(ValueError) as exc_info:
            OpenAI(base_url=base_url, api_key=api_key).copy(auth=AzureAuth(token_provider=lambda: "test_bearer_token"))
        assert str(exc_info.value) == "The `api_key` and `auth` arguments are mutually exclusive"
...
class TestAsyncOpenAI:
    ...
    async def test_refresh_auth_headers_token_async(self) -> None:
        ...
        client = AsyncOpenAI(base_url=base_url, auth=AsyncAzureAuth(token_provider=token_provider))
    ...
    def test_auth_mutually_exclusive_async(self) -> None:
        ...
        with pytest.raises(ValueError) as exc_info:
            AsyncOpenAI(base_url=base_url, api_key=api_key, auth=AsyncAzureAuth(token_provider=token_provider))
        assert str(exc_info.value) == "The `api_key` and `auth` arguments are mutually exclusive"
    ...
    async def test_copy_auth(self) -> None:
        ...
        client = AsyncOpenAI(base_url=base_url, auth=AsyncAzureAuth(token_provider=token_provider_1)).copy(
            auth=AsyncAzureAuth(token_provider=token_provider_2)
        )
        await client.refresh_auth_headers()
        assert client.auth_headers == {"Authorization": "Bearer test_bearer_token_2"}
    ...
    def test_copy_auth_mutually_exclusive_async(self) -> None:
        ...
        with pytest.raises(ValueError) as exc_info:
            AsyncOpenAI(base_url=base_url, api_key=api_key).copy(auth=AsyncAzureAuth(token_provider=token_provider))
        assert str(exc_info.value) == "The `api_key` and `auth` arguments are mutually exclusive"
